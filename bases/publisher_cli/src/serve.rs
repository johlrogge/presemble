...
const INJECT: &str = concat!(
    ...
    ".presemble-suggest-indicator::after{content:'S';position:absolute;top:-0.5rem;right:-0.5rem;width:1.2rem;height:1.2rem;background:#e8a838;color:#fff;border-radius:50%;font-size:0.7rem;font-weight:bold;display:flex;align-items:center;justify-content:center;cursor:pointer;}",
    ...
    ".presemble-suggest-diff{background:#fff8e1;border:1px solid #e8a838;border-radius:0.3rem;padding:0.3rem 0.5rem;margin:0.3rem 0;font-size:0.85rem;font-family:system-ui,sans-serif;}",
    ".presemble-suggest-diff del{color:#a33;text-decoration:line-through;}",
    ".presemble-suggest-diff ins{color:#2d7a3a;text-decoration:none;font-weight:bold;}",
    ...
    "var editBtn=document.createElement('button');editBtn.textContent='E Edit';",
    "var suggestBtn=document.createElement('button');suggestBtn.textContent='S Suggest';suggestBtn.style.position='relative';",
    ...
    "function update(){",
      ...
      "if(mode==='edit'){icon.textContent='E';icon.title='Edit mode \\u2014 click to change';}",
      "else if(mode==='suggest'){icon.textContent='S';icon.title='Suggest mode \\u2014 click to change';}",
    ...
    // Suggest mode state
    "var _suggestions=[];var _suggestIdx=0;var _suggestPreviewState=null;",
    ...
    "function _suggestCleanup(){",
      ...
      "});",
      "document.querySelectorAll('.presemble-suggest-diff').forEach(function(el){el.remove();});",
    ...
    "function _suggestFindTarget(sug){",
      ...
      "if(sug.target_type==='body'&&sug.search){",
        ...
        "for(var i=0;i<els.length;i++){if(els[i].textContent.indexOf(sug.search)!==-1){return els[i];}}",
      "}",
      "return null;",
    "}",
    "function _suggestRenderToolbar(){",
      ...
      "if(!_suggestToolbar){",
        ...
        "_suggestToolbar.innerHTML='<button class=\"presemble-suggest-prev\" title=\"Previous\">&#9664;</button>'",
          ...
          "+'<button class=\"presemble-suggest-preview\" title=\"Preview\">Preview</button>'",
    ...
    "function _suggestHighlight(){",
      ...
      "if(el){",
        ...
        "diffEl.className='presemble-suggest-diff';",
        "if(sug.target_type==='slot'){",
          "diffEl.innerHTML='<del>'+el.textContent+'</del> \\u2192 <ins>'+(sug.proposed_value||'')+'</ins>';",
        "}else if(sug.target_type==='body'){",
          "diffEl.innerHTML='<del>'+(sug.search||'')+'</del> \\u2192 <ins>'+(sug.replace||'')+'</ins>';",
        "}",
        "el.after(diffEl);",
      "}",
    ...
    "function _suggestTogglePreview(){",
      ...
      "if(_suggestPreviewState){",
        "if(sug.target_type==='slot'){el.textContent=_suggestPreviewState;}",
        "else if(sug.target_type==='body'){el.innerHTML=_suggestPreviewState;}",
        "_suggestPreviewState=null;",
        "el.classList.remove('presemble-suggest-preview-active');",
        "el.classList.add('presemble-suggest-active');",
        "var diffElOff=el.nextElementSibling;",
        "if(diffElOff&&diffElOff.classList.contains('presemble-suggest-diff')){diffElOff.style.display='';}",
        "if(previewBtn){previewBtn.classList.remove('active');}",
      "}else{",
        "if(sug.target_type==='slot'){",
          "_suggestPreviewState=el.textContent;",
        ...
        "}else if(sug.target_type==='body'&&sug.search&&sug.replace){",
          ...
          "var idx=txt.indexOf(sug.search);",
          "if(idx!==-1){",
            "el.textContent=txt.slice(0,idx)+sug.replace+txt.slice(idx+sug.search.length);",
          "}",
        "}",
        "el.classList.add('presemble-suggest-preview-active');",
        "el.classList.remove('presemble-suggest-active');",
        "var diffElOn=el.nextElementSibling;",
        "if(diffElOn&&diffElOn.classList.contains('presemble-suggest-diff')){diffElOn.style.display='none';}",
