...
fn render_article_with_dom_transformer() {
    ...
    context.insert("self", template::Value::Record(slot_graph));

    let html = template::render_template(template_src, &context)
        .expect("render should succeed");

    // Title with semantic class
    assert!(html.contains("self-title"), "title class in output: {html}");
    ...
    assert!(html.contains("self-author"), "author class in output: {html}");
    assert!(html.contains("johlrogge"), "author href in output: {html}");

    // Cover image with semantic class
    assert!(html.contains("self-cover"), "cover class in output: {html}");
...
fn render_template_missing_slot_produces_empty_not_error() {
    ...
    let template_src = r#"<div><presemble:insert data="self.missing" /></div>"#;
...
fn render_template_data_slot_absent_removes_block() {
    let template_src = r#"<template data-slot="self.cover"><img src="x" /></template>"#;
